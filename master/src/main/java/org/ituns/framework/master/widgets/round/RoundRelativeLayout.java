...
public class RoundRelativeLayout extends RelativeLayout {
    ...
    public RoundRelativeLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if(attrs != null) {
            ...
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.RoundRelativeLayout, 0, 0);
            for(int i = 0; i < ta.getIndexCount(); i++) {
                int attr = ta.getIndex(i);
                if(attr == R.styleable.RoundRelativeLayout_rrl_size_width) {
                    size_width = ta.getDimensionPixelSize(attr, -1);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_size_height) {
                    size_height = ta.getDimensionPixelSize(attr, -1);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_solid_color) {
                    solid_color = ta.getColor(attr, Color.TRANSPARENT);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_stroke_color) {
                    stroke_color = ta.getColor(attr, Color.TRANSPARENT);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_stroke_width) {
                    stroke_width = ta.getDimensionPixelSize(attr, -1);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_stroke_dash_gap) {
                    stroke_dash_gap = ta.getDimensionPixelSize(attr, 0);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_stroke_dash_width) {
                    stroke_dash_width = ta.getDimensionPixelSize(attr, 0);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_corners_radius) {
                    corners_radius = ta.getDimensionPixelSize(attr, -1);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_corners_radius_tl) {
                    corners_radius_tl = ta.getDimensionPixelSize(attr, -1);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_corners_radius_tr) {
                    corners_radius_tr = ta.getDimensionPixelSize(attr, -1);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_corners_radius_bl) {
                    corners_radius_bl = ta.getDimensionPixelSize(attr, -1);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_corners_radius_br) {
                    corners_radius_br = ta.getDimensionPixelSize(attr, -1);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_padding_top) {
                    padding_top = ta.getDimensionPixelSize(attr, 0);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_padding_left) {
                    padding_left = ta.getDimensionPixelSize(attr, 0);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_padding_right) {
                    padding_right = ta.getDimensionPixelSize(attr, 0);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_padding_bottom) {
                    padding_bottom = ta.getDimensionPixelSize(attr, 0);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_gradient_type) {
                    gradient_type = ta.getInt(attr, -1);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_gradient_alpha) {
                    gradient_alpha = ta.getInt(attr, 255);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_gradient_shape) {
                    gradient_shape = ta.getInt(attr, -1);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_gradient_dither) {
                    gradient_dither = ta.getBoolean(attr, false);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_gradient_radius) {
                    gradient_radius = ta.getDimensionPixelSize(attr, 0);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_gradient_center_x) {
                    gradient_center_x = ta.getFloat(attr, -1.0f);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_gradient_center_y) {
                    gradient_center_y = ta.getFloat(attr, -1.0f);
                } else if(attr == R.styleable.RoundRelativeLayout_rrl_gradient_orientation) {
