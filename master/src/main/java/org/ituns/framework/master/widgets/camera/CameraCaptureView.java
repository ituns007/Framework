...
public class CameraCaptureView extends FrameLayout {
    ...
    public CameraCaptureView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CameraCaptureView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void startPreview(LifecycleOwner owner, ProcessCameraProvider provider) {
        try {
            ...
            imageCapture = new ImageCapture.Builder()
                    ...
                    .setFlashMode(cameraConfiguration.flashMode())
                    .setTargetResolution(targetResolution)
                    .setTargetRotation(targetRotation)
                    .build();
    ...
    public final void takeCapture() {
        try {
            if(imageCapture == null) {
                Logcat.e("image capture is null.");
                return;
            }

            imageCapture.takePicture(executorHolder.get(), capturedCallback);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private final ImageCapture.OnImageCapturedCallback capturedCallback = new ImageCapture.OnImageCapturedCallback() {
        @Override
